...
export function generateInvoicePDF(invoice: Invoice): void {
  ...
  if (invoice.quoteNumber) {
    currentY += 5;
    doc.text(`Réf. Devis : ${invoice.quoteNumber}`, colLeftX, currentY);
  }
  ...
  // Format specific vehicle details if available, otherwise fallback
  let vehText = `Véhicule : ${invoice.clientVehicle || ''}`;
  if (invoice.vehicleBrand || invoice.vehicleModel) {
    const brand = invoice.vehicleBrand || '';
    const model = invoice.vehicleModel || '';
    vehText = `Véhicule : ${brand} ${model}`.trim();
  }
  doc.text(vehText, colRightX, currentY);

  if (invoice.vehicleRegistration || invoice.vehicleMileage) {
    let extraDetails = '';
    if (invoice.vehicleRegistration) extraDetails += `Immat : ${invoice.vehicleRegistration}`;
    if (invoice.vehicleMileage) {
      if (extraDetails) extraDetails += ' | ';
      extraDetails += `Km : ${invoice.vehicleMileage} km`;
    }
    currentY += 5;
    doc.text(extraDetails, colRightX, currentY);
  }
  ...
  // --- PUBLIC NOTES ---
  if (invoice.publicNotes && invoice.publicNotes.trim()) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8.5);
    doc.setTextColor(SECONDARY_COLOR[0], SECONDARY_COLOR[1], SECONDARY_COLOR[2]);
    doc.text('Observations / Remarques :', marginX, totalsYStart);
    
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(TEXT_DARK[0], TEXT_DARK[1], TEXT_DARK[2]);
    
    const lines = doc.splitTextToSize(invoice.publicNotes, 100);
    doc.text(lines, marginX, totalsYStart + 5.5);
  }
...
export function generateQuotePDF(quote: Quote): void {
  ...
  // Left Column - Quote specs
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
  doc.text(`DEVIS N° : ${quote.quoteNumber}`, colLeftX, currentY);
  ...
  currentY += 7;
  doc.text(`Date de devis : ${quote.date}`, colLeftX, currentY);
  currentY += 5;
  doc.text(`Valide jusqu'au : ${quote.validUntil || 'N/A'}`, colLeftX, currentY);
  ...
  if (quote.status === 'Accepted') {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.text('ACCEPTÉ', colLeftX + 6, currentY);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.text('REFUSÉ', colLeftX + 8, currentY);
  } else {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.text('EN ATTENTE', colLeftX + 4, currentY);
  }
  ...
  doc.setFontSize(11);
  doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
  doc.text('PROPOSITION POUR :', colRightX, currentY);
  ...
  currentY += 5;
  doc.setFont('helvetica', 'bold');
  doc.text(quote.clientName, colRightX, currentY);
  
  doc.setFont('helvetica', 'normal');
  currentY += 5;
  doc.text(`Tél : ${quote.clientPhone}`, colRightX, currentY);
  currentY += 5;
  doc.text(`Email : ${quote.clientEmail}`, colRightX, currentY);
  
  currentY += 5;
  doc.setFont('helvetica', 'oblique');
  
  // Format specific vehicle details if available, otherwise fallback
  let vehText = `Véhicule : ${quote.clientVehicle || ''}`;
  if (quote.vehicleBrand || quote.vehicleModel) {
    const brand = quote.vehicleBrand || '';
    const model = quote.vehicleModel || '';
    vehText = `Véhicule : ${brand} ${model}`.trim();
  }
  doc.text(vehText, colRightX, currentY);

  if (quote.vehicleRegistration || quote.vehicleMileage) {
    let extraDetails = '';
    if (quote.vehicleRegistration) extraDetails += `Immat : ${quote.vehicleRegistration}`;
    if (quote.vehicleMileage) {
      if (extraDetails) extraDetails += ' | ';
      extraDetails += `Km : ${quote.vehicleMileage} km`;
    }
    currentY += 5;
    doc.text(extraDetails, colRightX, currentY);
  }
  ...
  // --- PUBLIC NOTES ---
  if (quote.publicNotes && quote.publicNotes.trim()) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8.5);
    doc.setTextColor(SECONDARY_COLOR[0], SECONDARY_COLOR[1], SECONDARY_COLOR[2]);
    doc.text('Observations / Remarques :', marginX, totalsYStart);
    ...
    const lines = doc.splitTextToSize(quote.publicNotes, 100);
    doc.text(lines, marginX, totalsYStart + 5.5);
  }
  ...
  const totalHTAfterDiscount = subtotalHT - discountAmount;
  const taxAmount = totalHTAfterDiscount * (quote.taxRate / 100);
  const calculatedTotalTTC = totalHTAfterDiscount + taxAmount;
  ...
  if (quote.discount > 0) {
    currentY += 5.5;
    doc.setFont('helvetica', 'oblique');
    doc.text(`Remise (${quote.discount}%) :`, totalsX, currentY);
    doc.text(`-${discountAmount.toFixed(2)} ${currency}`, 180, currentY);
    doc.setFont('helvetica', 'normal');
  }

  currentY += 5.5;
  doc.text(`TVA (${quote.taxRate}%) :`, totalsX, currentY);
  doc.text(`${taxAmount.toFixed(2)} ${currency}`, 180, currentY);
  ...
  doc.setFontSize(10.5);
  doc.setTextColor(255, 255, 255);
  doc.text('TOTAL DEVIS TTC :', totalsX, currentY + 5.5);
  doc.text(`${calculatedTotalTTC.toFixed(2)} ${currency}`, 180, currentY + 5.5);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
  doc.text('Ce devis est valable pendant 30 jours.', 105, currentY, { align: 'center' });
  ...
  doc.text(`${settings?.companyName || 'Atelier Mécanique Mobile'} - Societé au capital de ${companyCapital} - ICE: ${companyIce} - Ville: ${companyCity}`, 105, currentY, { align: 'center' });
  currentY += 3.5;
  doc.text('Pour acceptation du devis, veuillez retourner ce document signé avec la mention "Bon pour Accord".', 105, currentY, { align: 'center' });

  // Download Action
  doc.save(`Devis_${quote.quoteNumber}.pdf`);
}

export function generateDeliveryNotePDF(document: Quote | Invoice): void {
  // Read saved settings
  let settings: any = null;
  try {
    const saved = localStorage.getItem('autocare_settings');
  ...
  const PRIMARY_COLOR = [31, 41, 55]; // Gray 800
  const SECONDARY_COLOR = [75, 85, 99]; // Gray 600
  const ACCENT_COLOR = [16, 185, 129]; // Emerald Green 500 (representing completed delivery)
  const LIGHT_BG = [243, 244, 246]; // Gray 100
  const TEXT_DARK = [17, 24, 39]; // Gray 900
  ...
  // --- BL SPEC INFO & CLIENT SPEC INFO ---
  const colLeftX = marginX;
  const colRightX = 115;
  const infoSectionY = currentY;

  // Left Column - BL metadata
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
  doc.text(`BON DE LIVRAISON`, colLeftX, currentY);
  ...
  const baseNumber = isInvoice ? (document as Invoice).invoiceNumber : (document as Quote).quoteNumber;
  const deliveryNo = baseNumber.replace('DEVIS-', 'BL-').replace('FACT-', 'BL-');
  
  doc.text(`N° Livraison : ${deliveryNo}`, colLeftX, currentY);
  currentY += 5;
  doc.text(`Date de livraison : ${new Date().toISOString().split('T')[0]}`, colLeftX, currentY);
  currentY += 5;
  doc.setFont('helvetica', 'bold');
  if (isInvoice) {
    doc.text(`Réf. Facture : ${baseNumber}`, colLeftX, currentY);
  } else {
    doc.text(`Réf. Devis : ${baseNumber}`, colLeftX, currentY);
  }
  doc.setFont('helvetica', 'normal');
  ...
  doc.setFontSize(11);
  doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
  doc.text('RÉCEPTIONNAIRE :', colRightX, currentY);
  ...
  currentY += 5;
  doc.setFont('helvetica', 'bold');
  doc.text(document.clientName, colRightX, currentY);
  
  doc.setFont('helvetica', 'normal');
  currentY += 5;
  doc.text(`Tél : ${document.clientPhone}`, colRightX, currentY);
  currentY += 5;
  doc.text(`Email : ${document.clientEmail}`, colRightX, currentY);
  
  currentY += 5;
  doc.setFont('helvetica', 'oblique');
  
  // Format specific vehicle details if available, otherwise fallback
  let vehText = `Véhicule : ${document.clientVehicle || ''}`;
  if (document.vehicleBrand || document.vehicleModel) {
    const brand = document.vehicleBrand || '';
    const model = document.vehicleModel || '';
    vehText = `Véhicule : ${brand} ${model}`.trim();
  }
  doc.text(vehText, colRightX, currentY);

  if (document.vehicleRegistration || document.vehicleMileage) {
    let extraDetails = '';
    if (document.vehicleRegistration) extraDetails += `Immat : ${document.vehicleRegistration}`;
    if (document.vehicleMileage) {
      if (extraDetails) extraDetails += ' | ';
      extraDetails += `Km : ${document.vehicleMileage} km`;
    }
    currentY += 5;
    doc.text(extraDetails, colRightX, currentY);
  }
  ...
  doc.text('Désignation / Prestation / Pièce', marginX + 3, currentY + 3.5);
  doc.text('Type', 110, currentY + 3.5);
  doc.text('Qté Commandée', 135, currentY + 3.5);
  doc.text('Qté Livrée / Reçue', 170, currentY + 3.5);
  ...
  document.items.forEach((item, index) => {
    ...
    const cleanName = item.name.length > 50 ? item.name.slice(0, 47) + '...' : item.name;
    const catText = item.type === 'Part' ? 'Pièce' : 'Main d\'œuvre';

    doc.text(cleanName, marginX + 3, currentY + 5);
    doc.text(catText, 110, currentY + 5);
    doc.text(item.quantity.toString(), 145, currentY + 5, { align: 'center' });
    
    // Quantity delivered box with checkboxes
    doc.setDrawColor(200, 200, 200);
    doc.rect(178, currentY + 1.8, 4, 4);
    doc.text(`[   ] ${item.quantity}`, 173, currentY + 5);
  ...
  // --- PUBLIC NOTES ---
  if (document.publicNotes && document.publicNotes.trim()) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8.5);
    doc.setTextColor(SECONDARY_COLOR[0], SECONDARY_COLOR[1], SECONDARY_COLOR[2]);
    doc.text('Notes / Observations :', marginX + 5, currentY);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(TEXT_DARK[0], TEXT_DARK[1], TEXT_DARK[2]);
    const lines = doc.splitTextToSize(document.publicNotes, 170);
    doc.text(lines, marginX + 5, currentY + 4.5);
    
    currentY += 4.5 + (lines.length * 4.5) + 6;
  }
  ...
  doc.text('POUR L\'ATELIER (LIVREUR)', marginX + 5, currentY);
  doc.text('POUR LE CLIENT (RECEPTIONNAIRE)', colRightX + 5, currentY);

  currentY += 5;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(SECONDARY_COLOR[0], SECONDARY_COLOR[1], SECONDARY_COLOR[2]);
  doc.text('Cachet et signature :', marginX + 5, currentY);
  doc.text('Date, Nom & Signature précédés de', colRightX + 5, currentY);
  currentY += 4;
  doc.text('la mention manuscrite "REÇU CONFORME" :', colRightX + 5, currentY);

  // Boxes for signature
  doc.setDrawColor(200, 200, 200);
  doc.rect(marginX + 5, currentY + 2, 70, 28);
  doc.rect(colRightX + 5, currentY + 2, 70, 28);
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(PRIMARY_COLOR[0], PRIMARY_COLOR[1], PRIMARY_COLOR[2]);
  doc.text('Ce bon de livraison certifie la bonne livraison des pièces et prestations ci-dessus.', 105, currentY, { align: 'center' });
  ...
  doc.text(`${settings?.companyName || 'Atelier Mécanique Mobile'} - Societé au capital de ${companyCapital} - ICE: ${companyIce} - Ville: ${companyCity}`, 105, currentY, { align: 'center' });
  currentY += 3.5;
  doc.text('En cas de litige, veuillez mentionner vos réserves de manière détaillée sur ce document.', 105, currentY, { align: 'center' });

  // Download Action
  doc.save(`Bon_Livraison_${deliveryNo}.pdf`);
}
